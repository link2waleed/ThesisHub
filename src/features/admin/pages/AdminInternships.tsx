...
import { adminInternships, type AdminInternshipItem } from '@/data/adminData';

export default function AdminInternships() {
    const [data, setData] = useState(adminInternships);
    const [search, setSearch] = useState('');
    const [statusFilter, setStatusFilter] = useState<string>('all');
    const [deleteTarget, setDeleteTarget] = useState<AdminInternshipItem | null>(null);
    ...
    const columns: Column<AdminInternshipItem>[] = [
        {
            key: 'title',
            label: 'Title',
            sortable: true,
        ...
        { key: 'company', label: 'Company', sortable: true, render: (row) => <span className="text-sm text-slate-600 dark:text-slate-300">{row.company}</span> },
    ...
    return (
        <div className="space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                    <h1 className="text-xl font-bold text-slate-900 dark:text-slate-50" style={{ fontFamily: 'var(--font-display)' }}>Internship Management</h1>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{data.length} total internships</p>
                </div>
            ...
            <ConfirmModal
                ...
                onConfirm={() => {
                    if (deleteTarget) setData((d) => d.filter((i) => i.id !== deleteTarget.id));
                }}
